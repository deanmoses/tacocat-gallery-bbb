  ...
  Album.Model = Backbone.Model.extend({
	...
	fetchAlbum : function(path) {
		...
		// else the album is not in cache...
			...
			album = new Album.Model({fullPath : path});
			album.fetch({
	...
	Album.Views.Week = Backbone.View.extend({
		...
		render : function() {
			...
			var bodyHtml = app.renderTemplate('album_week_body', {album: this.model, thumbnails: thumbnailHtml});

			// Generate the layout HTML
			var html = app.renderTemplate('layout_main', {pageType : 'album week', header : headerHtml, body : bodyHtml});
	...
	Album.Views.Year = Backbone.View.extend({
		...
		render : function() {
			...
			// Generate the layout HTML
			var html = app.renderTemplate('layout_main', {pageType : 'album year', header : headerHtml, body : bodyHtml});
	...
	Album.Views.Root = Backbone.View.extend({
      ...
      render: function(layout) {
				...
				// Generate the layout HTML
				var html = app.renderTemplate('layout_main', {pageType : 'album root', header : headerHtml, body : bodyHtml});
	...
	Album.Views.Main = Backbone.View.extend({
      ...
      render: function(layout) {
	        	...
	        	if (albumType == "week") {
					...
					new Album.Views.Year({model: album,el: $('#main')}).render();
				}
				else if (albumType == "root") {
					new Album.Views.Root({model: album,el: $('#main')}).render();
				}
				else {
